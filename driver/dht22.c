...
ICACHE_FLASH_ATTR static inline float scale_humidity(DHTType sensor_type, int *data)
{
...
ICACHE_FLASH_ATTR static inline float scale_temperature(DHTType sensor_type, int *data)
{
	if(sensor_type == DHT11) {
		return (float) data[2];
	} else {
...
ICACHE_FLASH_ATTR char* DHTFloat2String(char* buffer, float value)
{
  os_sprintf(buffer, "%d.%d", (int)(value),(int)((value - (int)value)*100));
  return buffer;
}

ICACHE_FLASH_ATTR bool DHTRead(DHT_Sensor *sensor, DHT_Sensor_Data* output)
{
	int counter = 0;
	int laststate = 1;
	int i = 0;
...
ICACHE_FLASH_ATTR bool DHTInit(DHT_Sensor *sensor)
{
